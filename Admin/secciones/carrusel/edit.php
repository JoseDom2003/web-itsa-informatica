...
$stmt = $pdo->prepare("SELECT * FROM tbl_carrusel WHERE id_carrusel = ?");
$stmt->execute([$id]);
    ...
    if ($imagen && $imagen['error'] === UPLOAD_ERR_OK) {
        ...
        if (str_starts_with($mime, 'image/')) {
            ...
            if (move_uploaded_file($tmpName, $uploadDir . $newName)) {
                ...
                $stmt = $pdo->prepare(
                    "UPDATE tbl_carrusel SET imagen = ?, titulo = ?, subtitulo = ? WHERE id_carrusel = ?"
                );
                $stmt->execute([$newName, $titulo, $subtitulo, $id]);

                // Redirigir a config-carrusel.php
                header("Location: /Admin/dashboard.php");
                exit();
            } else {
                echo "<div class='alert alert-danger text-center'>Error al mover la nueva imagen.</div>";
            }
        } else {
            echo "<div class='alert alert-danger text-center'>El archivo subido no es una imagen válida.</div>";
        }
    } else {
        // Si no se subió una nueva imagen, actualizar solo título y subtítulo
        $stmt = $pdo->prepare(
            "UPDATE tbl_carrusel SET titulo = ?, subtitulo = ? WHERE id_carrusel = ?"
        );
        $stmt->execute([$titulo, $subtitulo, $id]);
...
<head>
    ...
    <title>Editar Elemento del Carrusel</title>
...
<body>
    <div class="container d-flex justify-content-center align-items-center" style="padding-top: 15rem;">
        <div class="card p-4 shadow" style="width: 400px; border-radius: 10px;">
            <h2 class="text-center mb-4">Editar Elemento del Carrusel</h2>
            <form action="" method="POST" enctype="multipart/form-data" class="row g-3">
                ...
                <div class="col-12">
                    ...
                    <input type="text" class="form-control form-control-sm" id="subtitulo" name="subtitulo" value="<?php echo htmlspecialchars($item['subtitulo']); ?>" required>
                </div>
                <!-- Botón de Enviar -->
